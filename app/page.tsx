...
export default function Home() {
  ...
  const downloadAtsUpdatedPdf = async () => {
    ...
    try {
      const res = await axios.post("/api/inject-skills-to-cv", {
        cv_text: baselineText,
        skills_to_add: injectedSkills
      });
      
      // CRITICAL HOTFIX: Safely looks for the base64 data regardless of how Axios or the backend wraps it
      const targetedPdfData = res.data?.pdf_data || res.data?.data?.pdf_data;
      ...
      if (targetedPdfData) {
        ...
        alert("Compilation fault running matrix design rules.");
      }
    } catch (err) {
      alert("Error building download stream from the source file.");
    } finally {
  ...
  const handleGenerate = async () => {
    ...
    if (mode === "proposal") {
      ...
      try {
        ...
        setCvTextPreview("Error rendering original document canvas mapping."); 
      } finally { setLoading(false); }
    } else {
      setMatchData(null);
      setInjectedSkills([]); 
      try {
        ...
        alert("ATS Matrix engine evaluation timeout."); 
      } finally { setLoading(false); }
    }
  };
